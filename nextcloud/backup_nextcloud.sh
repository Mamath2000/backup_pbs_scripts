...
# Obtenir l'ID du conteneur Docker
...
sftp_upload() {
    ...
    if [[ "$SFTP_ENABLED" != "true" ]]; then
        log_info "SFTP désactivé, transfert ignoré"
        return 0
    fi

    log_info "Début du transfert SFTP: $(basename "$local_file")"

    # Extraction du répertoire distant
    local remote_dir=$(dirname "$remote_file")
    
    while [[ $retry_count -lt $SFTP_MAX_RETRIES ]]; do
        log_debug "Tentative de transfert SFTP $((retry_count + 1))/$SFTP_MAX_RETRIES"

        # Création du répertoire distant s'il n'existe pas et transfert du fichier
        local sftp_commands="
-mkdir $remote_dir
put \"$local_file\" \"$remote_file\"
quit"

        if echo "$sftp_commands" | sftp -i "$SFTP_KEY_PATH" -P "$SFTP_PORT" \
           -o StrictHostKeyChecking=no \
           -o UserKnownHostsFile=/dev/null \
           -o PasswordAuthentication=no \
           -o PreferredAuthentications=publickey \
           -o LogLevel=ERROR \
           "${SFTP_USER}@${SFTP_HOST}" 2>>"$LOG_FILE" >/dev/null
        then
            log_info "Transfert SFTP réussi vers: $remote_file"
            return 0
        else
            retry_count=$((retry_count + 1))
            log_warn "Échec du transfert SFTP (tentative $retry_count/$SFTP_MAX_RETRIES)"

            if [[ $retry_count -lt $SFTP_MAX_RETRIES ]]; then
                log_info "Nouvelle tentative dans ${SFTP_RETRY_DELAY}s..."
                sleep "$SFTP_RETRY_DELAY"
            fi
        fi
    done

    log_error "Échec définitif du transfert SFTP après $SFTP_MAX_RETRIES tentatives"
    return 1
}

cleanup_remote_backups() {
    if [[ "$SFTP_ENABLED" != "true" ]]; then
        log_debug "SFTP désactivé, nettoyage distant ignoré"
        return 0
    fi

    log_info "Nettoyage des anciennes sauvegardes sur le serveur SFTP (conservation: ${SFTP_DAYS_TO_KEEP} jours)"

    # Commandes SFTP pour lister et supprimer les fichiers anciens
    local sftp_commands="
cd $SFTP_REMOTE_DIR
ls -la
quit"
    ...
    local file_list=$(echo "$sftp_commands" | sftp -i "$SFTP_KEY_PATH" -P "$SFTP_PORT" \
        -o StrictHostKeyChecking=no \
        -o UserKnownHostsFile=/dev/null \
        -o PasswordAuthentication=no \
        -o PreferredAuthentications=publickey \
    ...
    local cutoff_date=$(date -d "${SFTP_DAYS_TO_KEEP} days ago" +%Y%m%d)
    
    # Analyse des fichiers de sauvegarde Nextcloud
    while IFS= read -r line; do
        if [[ "$line" =~ ^-.*[0-9]{12}_nextcloud_.* ]]; then
            local filename=$(echo "$line" | awk '{print $NF}')
            # Extraction de la date du nom de fichier (YYYYMMDDHHMM)
            if [[ "$filename" =~ ^([0-9]{8})[0-9]{4}_nextcloud_ ]]; then
                local file_date="${BASH_REMATCH[1]}"
                if [[ "$file_date" < "$cutoff_date" ]]; then
                    log_info "Suppression du fichier distant ancien: $filename"
...
quit"
                    ...
                    if echo "$delete_command" | sftp -i "$SFTP_KEY_PATH" -P "$SFTP_PORT" \
                        -o StrictHostKeyChecking=no \
                        -o UserKnownHostsFile=/dev/null \
                        -o PasswordAuthentication=no \
                        -o PreferredAuthentications=publickey \
                        -o LogLevel=ERROR \
                        "${SFTP_USER}@${SFTP_HOST}" 2>/dev/null >/dev/null; then
                        deleted_count=$((deleted_count + 1))
                        log_debug "Fichier distant supprimé: $filename"
                    else
                        log_warn "Échec de la suppression du fichier distant: $filename"
                    fi
                fi
            fi
        fi
    ...
    log_info "Suppression de $deleted_count fichier(s) ancien(s) sur le serveur SFTP"
}
...
perform_database_dump() {
    ...
    if [[ "$TEST_MODE" == "true" ]]; then
        log_info "MODE TEST: Création d'un fichier dummy"
...
create_dummy_backup() {
    ...
    if dd if=/dev/urandom of="$backup_file" bs=1M count="$TEST_DUMMY_SIZE_MB" 2>>"$LOG_FILE"; then
        log_info "Fichier dummy créé: $(basename "$backup_file") (${TEST_DUMMY_SIZE_MB}MB)"
